...
(function () {
  ...
  function renderGrid(rows) {
    $("grid").innerHTML = rows.map(function (i) {
      ...
      // .none = keine Freigabe (Warnung). Nur bei aktivem FSK-Feature. Gesperrt
      // zeigt nur die gruene Umrandung, KEIN Schloss-Symbol (User-Wunsch).
  ...
  function saveFsk(id, rating, badge) {
    fetch("/api/fsk/write", { method: "POST", headers: { "Content-Type": "application/json" },
      ...
      .then(function (res) {
        ...
        render();   // Karten neu aufbauen - kein fragiles className-Patchen mehr
